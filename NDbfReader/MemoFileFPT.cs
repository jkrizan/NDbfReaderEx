...
{
    public class MemoFileFPT : MemoFileBase
    {
        ...
        public MemoFileFPT(Stream stream, Encoding encoding, MemoFileType memoType, StrictHeader? strictHeader)
            : base(stream, encoding)
        {
            ...
            switch (memoType)
            {
                case MemoFileType.DBT_Ver3:
                    ...
                    {
                        throw new Exception("DBT MemoStream length too short! [header ver3] [" + stream.Length +
                                            " < 512]");
                    }
                    break;
                ...
                case MemoFileType.FPT_Ver3:
                    if (stream.Length < 64)
                    {
                        throw new Exception("DBT MemoStream length too short! [header ver4] [" + stream.Length +
                                            " < 64]");
                    }
                    break;
        ...
        private bool CheckDbtFileAdmLength(Stream stream)
        {
            ...
            {
                reader.BaseStream.Position = 6;
                blockLength = reader.ReadInt16(); // FP:
            }
            else
            {
                reader.BaseStream.Position = 20;
                blockLength = reader.ReadInt16(); // dBase3:512
            }
        ...
        private byte[] ReadMemoArrayFPT(int blockNo)
        {
            // return content of memo in 'memoBytes' and size of useful bytes
            byte[] memoBytes = null;

            var reader = new BigEndianBinaryReader(stream); // don't use using '(BinaryReader reader...' because 'using' dispose 'stream' too!
            ...
            if ((int) signal[3] == 1)
            {
                ...
                if ((memoLen < 1) || (memoLen > maxBlobSize))
                {
                    throw new Exception(String.Format(
                        "ReadMemoArrayFPT({0}): memo signal OK, but memo length error! [{1}/1..{2}]", blockNo,
                        memoLen, maxBlobSize));
                }
            ...
            {
                throw ExceptionFactory.CreateArgumentException("blockNo",
                    "ReadMemoArrayFPT({0}) invalid memo type {1}", blockNo, (int) (signal[0]));
            }
        ...
        private byte[] ReadMemoArray4(int blockNo)
        {
            ...
            BinaryReader
                reader = new BinaryReader(
                    stream); // don't use using '(BinaryReader reader...' because 'using' dispose 'stream' too!
            reader.BaseStream.Position = (blockNo * blockSize);
            ...
            if (isDbt4MemoBlockSignal(signal))
            {
                ...
                {
                    throw new Exception(String.Format(
                        "ReadMemoArray4({0}): memo signal OK, but memo length error! [{1}/1..{2}]", blockNo,
                        memoLen, maxBlobSize));
                }
            ...
            {
                throw ExceptionFactory.CreateArgumentException("blockNo",
                    "ReadMemoArray4({0}) invalid block number or wrong fileformat or file corrupted!", blockNo);
            }
        ...
        public MemoFileType RetrieveDbtVersion(Stream stream)
        {
            if (stream.Length < 64)
            {
                ...
                throw new Exception("RetrieveDbtVersion: DBT MemoStream length too short! [" + stream.Length +
                                    " < 64]");
            }
            ...
            reader.BaseStream.Position = 20;
            int blockLength =
                reader.ReadInt16(); // dBase3:512, dBase4: (1-512)*54  http://www.dbase.com/help/IDE/IDH_IDE_SET_MBLOCK.htm
